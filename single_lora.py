...
class OrthLorasInjectedLinear(nn.Module):
    def __init__(self, in_features, out_features, bias, linear_state_dict, lora_list, orthogonal=False):
...
def main():
    ...
    parser.add_argument("--config", type=str, default="configs/single/kimoju.toml")
    parser.add_argument("--dropout", type=float, default=0.5)
    parser.add_argument("--output_path", type=str, default="./output_single_lora")
    ...
    output_path = os.path.join(args.output_path, config["save_folder_name"], f"dropout_{args.dropout}")
        ...
        for fullname, module in nms:
            if fullname in loras_dict:
                *path, name = fullname.split(".")
                ...
                tmp = OrthLorasInjectedLinear(parent._modules[name].in_features, parent._modules[name].out_features, not parent._modules[name].bias == None, parent._modules[name].state_dict(), loras_dict[fullname], False).to(parent._modules[name].weight.device).to(parent._modules[name].weight.dtype)
